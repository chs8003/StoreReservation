...
const utilList_login2 = document.querySelector(".utilList_login2");
  ...
  .then((data) => {
    ...
    if (data.userName) {
      let btn = document.createElement("button");
       utilList_login2.addEventListener("click", function () {
    ...
    let UserName2 = document.querySelector(".utilList_User2");
...
const url1 = `/api/store?adminNo=${uni1}`;
fetch(url1)
  .then((res) => {
    return res.json();
  })
  .then((data) => {
    list.adminCafe = data.adminCafe;
    list.storePhone = data.storePhone;
  })
  .catch((err) => console.log(err));
...
number.forEach((item, index) => {
  item.addEventListener("click", function (e) {
    ...
    <div class="count" </div>`;
    list.party_size = `${Number(e.target.innerText[0])}`;
...
function sent() {
  list.timeAdded = getCurrentDateTime();
   fetch(`/add/${uni1}`, {
    method: "post",
    ...
    .then((json) =>{
		alert("대기열 추가에 성공하였습니다") ;
		location.href = "/mainpage";
	})
    .catch((error) => {
      alert("대기열이 중복됩니다. 예약에 실패하였습니다", error);
    });
}
