      ...
      if (array.length > 1) {
        ...
        data.forEach((item) => {
          if (array.includes(item.addressName.split(" ")[1])) {
            ...
            let reservable = item.reservable;
            let currentqueueNumber = item.currentqueueNumber;
            let foodType = item.foodType;
            let review = item.reviewCount; 
            ...
            <p class="currentqueueNumber">${currentqueueNumber}</p>
            <p>팀</p>
          </div>
          </div>`;
